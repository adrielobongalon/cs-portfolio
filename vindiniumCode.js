...
//  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function richTest2() {
            if (weaklings.length > 1) {
                for (i = 0; i < weaklings.length; i++) {
                    if (weaklings[i].mineCount != weaklings[0].mineCount) {
                        return true;
                    }
                }
                return false;
        ...
        var richestWeakling;                                                    // richest weakling. if there are multiple, it only identifies one. used to calculate idealTarget
        var richestWeaklings = [];                                              // just in case the top few weaklings, but not all weaklings, are equally rich
        if (weaklings.length > 0) {
            richestWeakling = weaklings[0];
            for (i = 0; i < weaklings.length; i++) {
                if (richestWeakling.mineCount < weaklings[i].mineCount) {
                    richestWeakling = weaklings[i];
                }
            }
            richestWeaklings.push(richestWeakling);                             // one of the richest weaklings is obviously the most rich one
            for (i = 0; i < weaklings.length; i++) {
                if (weaklings[i].mineCount === richestWeaklings[0] && weaklings[i].id != richestWeaklings[0].id) {
                    richestWeaklings.push(weaklings[i]);                        // if there's anyone just as rich as the richest, then they're also one of the richest
                }
            }
            if (richestWeaklings.length === 0) {
                richestWeakling = "none";
            }
            else if (richestWeaklings.length === 1) {
                richestWeakling = richestWeaklings[0];
            }
            else if (richestWeaklings.length > 1) {
                richestWeakling = richestWeaklings[0];
                for (i = 0; i < richestWeaklings.length; i++) {
                    if (bot.findDistance(myPos, [richestWeaklings[i].pos.x, richestWeaklings[i].pos.y]) < bot.findDistance(myPos, [richestWeakling.pos.x, richestWeakling.pos.y])) {
                        richestWeakling = richestWeaklings[i];
                    }
                }
            }
        }
        ...
        var idealTarget;                                                        // richest weakling. if there are multiple, it takes the closest. if there are none, then is "none"
        var ouranWeaklings = [];                                                // initialises ouranWeakling (do you get the weeb reference?), opponent with less hp and most mines
        // if (weaklings.length === 0) {
        //     idealTarget = "none";
        // }
        // else if (weaklings.length === 1) {
        //     idealTarget = weaklings[0];
        // }
        // else if (weaklings.length > 1) {
        //     for (i = 0; i < weaklings.length; i++) {
        //         if (richestWeakling.mineCount < enemyBots[i].mineCount) {
        //             richestWeakling = enemyBots[i];
        //         }
        //     }
        //     ouranWeaklings.push(richestEnemy);                                  // the richest enemy of all obviously consists of the richest enemy
        //     for (i = 0; i < enemyBots.length; i++) {
        //         if (enemyBots[i].mineCount === richestEnemies[0]) {             // if there's anyone just as rich as the richest, then they're also one of the richest
        //             richestEnemies.push(enemyBots[i]);
        //         }
        //     }
        // }
        
        console.log("ouran kid: " + ouranWeaklings.id);
...
//      ----------------------------------------
        ...
        var task;
        if (bot.yourBot.life <= 50 && nearestFoolishMortal.life <= bot.yourBot.life + 10 &&
